...
var CDNURL =
  "https://hqrvipeczxkthmaeywym.supabase.co/storage/v1/object/public/videos";
...
const datosjuegos = async () => {
  const { data, error } = await supabase
    .from("productos")
    .select(
      "*, gender(genero) , dev(developer), fabricante(maker), platforms(plataforma)"
    )
    .match({ id: productoId });
  if (error) {
    console.log(error);
...
const videoGames = async () => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    newDiv.innerHTML = `
      <iframe  id="myVideo" width="560" height="315" src="${data[i].uploads}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe> 
    `;
